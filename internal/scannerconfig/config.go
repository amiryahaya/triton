...
var profiles = map[string]ScanProfile{
	...
	"standard": {
		...
		Modules: []string{"certificates", "keys", "packages", "libraries", "binaries", "scripts", "webapp", "configs", "containers", "certstore", "database", "deps", "web_server", "vpn", "password_hash", "deps_ecosystems", "mail_server", "dnssec", "netinfra", "messaging", "db_atrest", "archive", "ftps", "ssh_cert", "ldif", "python_ast"},
		Depth:   10,
		Workers: 8,
	},
	"comprehensive": {
		...
		Modules: []string{"certificates", "keys", "packages", "libraries", "binaries", "kernel", "scripts", "webapp", "configs", "processes", "network", "protocol", "containers", "certstore", "database", "hsm", "ldap", "codesign", "deps", "web_server", "vpn", "container_signatures", "password_hash", "auth_material", "deps_ecosystems", "service_mesh", "xml_dsig", "mail_server", "oci_image", "dnssec", "vpn_runtime", "netinfra", "firmware", "messaging", "db_atrest", "secrets_mgr", "supply_chain", "kerberos_runtime", "enrollment", "fido2", "blockchain", "helm_chart", "asn1_oid", "java_bytecode", "dotnet_il", "ebpf_trace", "tpm", "uefi", "archive", "tls_observer", "ftps", "ssh_cert", "ldif", "python_ast"},
		Depth:   -1, // unlimited
		Workers: 16,
	},
}
...
func Load(profile string) *Config {
	...
	for _, mod := range p.Modules {
		...
		case "ftps", "ssh_cert":
			// ftps and ssh_cert need network targets like protocol does
			hasNetwork := false
			for _, t := range targets {
				if t.Type == model.TargetNetwork {
					hasNetwork = true
